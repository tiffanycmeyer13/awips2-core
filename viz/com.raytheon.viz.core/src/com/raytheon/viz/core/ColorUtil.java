 ...
 * Apr 22, 2022 8791       mapeters    Ensure descriptors are compatible before
 *                                     reusing resource color from another pane
 *
 * </pre>
 *
 * @author chammack
...
public class ColorUtil {
    ...
    private static final String COLOR_PREF = "defaultGraphicColor";
    ...
    private static void addColorListColorMap(List<RGB> colorList,
        ...
        for (RGB color : colorList) {
            int prevCount = colorCountMap.getOrDefault(color, 0);
            colorCountMap.put(color, prevCount + 1);
        }
    }
    ...
    private static synchronized void checkAndLoadColors() {
        ...
        HierarchicalPreferenceStore prefs = CorePlugin.getDefault()
                .getPreferenceStore();

        String[] names = prefs.getStringArray(COLOR_PREF);
        colors = Arrays.stream(names).map(RGBColors::getRGBColor)
                .toArray(RGB[]::new);
    }
    ...
    private static Map<RGB, Integer> getColorCountMap() {
        ...
        Map<RGB, Integer> colorCountMap = new LinkedHashMap<>();
    ...
    public static RGB getNewColor(IDescriptor descriptor, ResourcePair pair) {
        ...
        if (pair != null && pair.getLoadProperties() != null
    ...
    public static RGB getNewColor(IDisplayPane[] displayPanes,
        ...
        for (IDisplayPane displayPane : displayPanes) {
            ...
            final ColorableCapability cap = oldRp.getLoadProperties()
                    .getCapabilities().getCapability(oldRp.getResourceData(),
                            ColorableCapability.class);
            pair.getLoadProperties().getCapabilities().addCapability(cap);

            /*
             * Since they now share the capability, we need to forward the
             * change event.
             */
            final AbstractResourceData rd = pair.getResourceData();
            oldRp.getResourceData().addChangeListener((type, object) -> {
                if (object == cap) {
    ...
    private static List<RGB> getUsedColors(IDescriptor descriptor) {
        ...
        List<RGB> usedColors = new ArrayList<>();
        ...
        synchronized (rl) {
            for (ResourcePair rp : rl) {
                if (rp.getLoadProperties().getCapabilities()
                        .hasCapability(ColorableCapability.class)) {
                    RGB color = rp.getLoadProperties().getCapabilities()
