...
public class ObStationDao extends CoreDao {
    ...
    public ObStation queryByIcao(String icao) throws DataAccessLayerException {
        ObStation station = null;
        List<?> obs = queryBySingleCriteria("gid",
                ObStation.createGID(ObStation.CAT_TYPE_ICAO, icao));
        if (obs != null && !obs.isEmpty()) {
            station = (ObStation) obs.get(0);
        }
        return station;
     ...
     */
    public ObStation queryByWmoIndex(Integer wmoIndex)
            throws DataAccessLayerException {
        ObStation station = null;
        if (wmoIndex != null) {
            List<?> obs = queryBySingleCriteria("gid", ObStation.createGID(
                    ObStation.CAT_TYPE_SFC_FXD, wmoIndex.toString()));
            if (obs != null && !obs.isEmpty()) {
                station = (ObStation) obs.get(0);
            }
        }
    ...
    public List<ObStation> queryByCountry(String country)
            throws DataAccessLayerException {
     ...
     */
    @SuppressWarnings("unchecked")
    public List<ObStation> queryByState(String state)
            throws DataAccessLayerException {
     ...
     */
    @SuppressWarnings("unchecked")
    public List<ObStation> queryByStationId(String stationId)
            throws DataAccessLayerException {
    ...
    public List<ObStation> queryBySpatialPolygon(List<Coordinate> coords) {
        ...
        if (coords.size() > 2) {
            ...
            sql.append(
                    "SELECT icao,wmoindex FROM awips.spatial_obs_stations WHERE stationgeom && ST_GeomFromText('POLYGON((");
            ...
            for (Object name : names) {
                if (((Object[]) name)[0] == null) {
                    ...
                    stationEq.add(
                            Expression.eq("wmoIndex", ((Object[]) name)[1]));
                } else {
