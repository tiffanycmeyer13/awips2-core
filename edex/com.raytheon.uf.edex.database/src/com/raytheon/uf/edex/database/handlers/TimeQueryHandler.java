...
public class TimeQueryHandler implements IRequestHandler<TimeQueryRequest> {
    ...
    public TimeQueryHandler(IDaoConfigFactory daoConfigFactory) {
    ...
    public List<DataTime> handleRequest(TimeQueryRequest request)
        ...
        if (request.getSimDate() != null) {
            ...
            timeConstraint.setConstraintValue(
                    new DataTime(request.getSimDate()).toString());
            map.put(REF_TIME, timeConstraint);
        }

        String database = PluginFactory.getInstance()
                .getDatabase(request.getPluginName());
        Class<?> clazz = PluginFactory.getInstance()
                .getPluginRecordClass(request.getPluginName());
        String classname = PluginFactory.getInstance()
                .getPluginRecordClassName(request.getPluginName());
        List<DataTime> times = null;

        CoreDao dao = new CoreDao(daoConfigFactory.forClass(database, clazz));
        ...
        if (binOffset != null) {
            ...
            times = new ArrayList<>(50);
            Date prevDate = null;
            while (latestTime != null && !latestTime.isEmpty()) {
                DataTime normalTime = binOffset
                        .getNormalizedTime(latestTime.get(0));
                ...
                if ((prevDate != null) && prevDate.equals(date)) {
                     ...
                     */
                    statusHandler.error(
                            "Preventing infinite time query recursion on plugin ["
                                    + pluginName
                ...
                map.get(REF_TIME)
                        .setConstraintValue(new DataTime(date).toString());
    ...
    private DatabaseQuery buildQuery(String classname,
        ...
        for (Entry<String, RequestConstraint> entry : map.entrySet()) {
    ...
    private List<DataTime> runQuery(CoreDao dao, DatabaseQuery query)
        ...
        List<DataTime> times = new ArrayList<>(results.size());
